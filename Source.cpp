...
public:
	...
	void update()
	{
		...
		// Move the creature too its new position
		circle.move({vx, vy});
	}
};
...
int main()
{
	...
	sf::RenderWindow window{ {constants::window_width, constants::window_height}, "Random Walk"s };
	...
	while (window.isOpen())
	{
		...
		// If the user presses "Escape", or clicked on "close", we close the window
